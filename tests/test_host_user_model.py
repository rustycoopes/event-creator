...
resolves against `host.users` as created by the Host repo's own migrations, using only the
...
async def test_host_user_selects_the_row_inserted_via_raw_sql(db_session: AsyncSession) -> None:
    user_id = await create_host_user(
        db_session,
        email="readonly-mapping@example.com",
        phone_number="+15550001111",
        dark_mode=True,
        nav_collapsed_groups={"event-creator": True},
    )
    ...
    assert host_user.nav_collapsed_groups == {"event-creator": True}


async def test_host_user_nav_collapsed_groups_defaults_to_empty_dict(
    ...
    assert host_user.nav_collapsed_groups == {}
...
async def test_host_user_mapping_only_declares_the_columns_it_reads(db_session: AsyncSession) -> None:
    ...
    assert mapped_columns == {"id", "email", "phone_number", "dark_mode", "nav_collapsed_groups"}
