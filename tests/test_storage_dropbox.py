"""Tests for the Dropbox OAuth connect/disconnect flow (ported, #93).

Mirrors tests/test_storage_google_drive.py's adaptation - see that module's docstring. No live
Dropbox credentials are touched.
"""
...
class FakeDropboxOAuth2:
    """Stands in for the Dropbox BaseOAuth2 client - never calls Dropbox."""
...
async def _dropbox_connect(
    ...
    fake: FakeDropboxOAuth2,
    *,
    code: str = "fake-code",
) -> None:
    _override(fake)
    try:
        auth = await client.post("/api/v1/storage-config/dropbox/auth", cookies=cookies)
...
async def test_auth_returns_dropbox_consent_url_with_correct_params(
    ...
    try:
        response = await client.post(
            "/api/v1/storage-config/dropbox/auth", cookies=cookies
        )
    finally:
        app.dependency_overrides.pop(get_dropbox_oauth_client, None)
        app.dependency_overrides.pop(get_cipher_factory, None)
...
async def test_auth_builds_redirect_uri_from_configured_setting_not_request_host(
    ...
    """Regression test for issue #201 (mirrors #200's Google Drive test): Dropbox rejects a
    redirect_uri that doesn't exactly match a URI registered on the app. Deriving it from the
    incoming request's Host header meant the value silently changed whenever the LB domain or
    backend service changed - the fix builds it from the fixed, per-environment
    DROPBOX_OAUTH_REDIRECT_URI setting instead. The test client's Host header is `testserver`, so
    if this ever regresses back to deriving from the request, the assertion below (which pins the
    exact configured value) fails."""
    ...
    app.dependency_overrides[get_cipher_factory] = lambda: (lambda: _CIPHER)
    try:
        response = await client.post("/api/v1/storage-config/dropbox/auth", cookies=cookies)
    finally:
        app.dependency_overrides.pop(get_cipher_factory, None)
    ...
    assert query["redirect_uri"][0] == settings.dropbox_oauth_redirect_uri
...
async def test_auth_fails_fast_when_redirect_uri_not_configured(
    ...
    """Regression test: a missing DROPBOX_OAUTH_REDIRECT_URI should be caught in /auth, before
    sending the user through the whole Dropbox consent flow only to have Dropbox reject it."""
    from app.core.config import get_settings
    from app.main import app
    ...
    try:
        with pytest.MonkeyPatch.context() as mp:
            mp.setenv("DROPBOX_OAUTH_REDIRECT_URI", "")
            get_settings.cache_clear()
            response = await client.post(
                "/api/v1/storage-config/dropbox/auth", cookies=cookies
            )
...
async def test_callback_stores_encrypted_tokens_and_sets_onboarding_flag(
    ...
    fake_dropbox_client: FakeDropboxOAuth2,
) -> None:
...
async def test_callback_marks_config_connected_for_subsequent_reads(
    ...
    await _dropbox_connect(client, cookies, fake_dropbox_client)
...
async def test_onboarding_flag_stays_true_after_disconnect(
    ...
    try:
        disconnect = await client.post(
            "/api/v1/storage-config/dropbox/disconnect", cookies=cookies
        )
    finally:
        _clear_overrides()
...
async def test_callback_rejects_tampered_state(
    ...
    _override(fake_dropbox_client)
    try:
        await client.post("/api/v1/storage-config/dropbox/auth", cookies=cookies)
